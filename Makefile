...
EXTENSION_FLAGS=\
-DDUCKDB_EXTENSION_NAMES="duckdb_pgwire" \
-DDUCKDB_EXTENSION_${EXTENSION_NAME}_PATH="$(PROJ_DIR)" \
-DDUCKDB_EXTENSION_${EXTENSION_NAME}_LOAD_TESTS=1 \
-DDUCKDB_EXTENSION_${EXTENSION_NAME}_INCLUDE_PATH="$(PROJ_DIR)include/duckpg" \
...
#### Main build
# For regular CLI build, we link the duckdb_pgwire extension directly into the DuckDB executable
CLIENT_FLAGS=-DDUCKDB_EXTENSION_${EXTENSION_NAME}_SHOULD_LINK=1

debug:
	mkdir -p  build/debug && \
	cmake $(GENERATOR) $(BUILD_FLAGS) $(CLIENT_FLAGS) -DCMAKE_BUILD_TYPE=Debug -S ./third_party/duckdb/ -B build/debug && \
	cmake --build build/debug --config Debug

release:
	mkdir -p build/release && \
	cmake $(GENERATOR) $(BUILD_FLAGS)  $(CLIENT_FLAGS)  -DCMAKE_BUILD_TYPE=Release -S ./third_party/duckdb/ -B build/release && \
...
#### Client tests
DEBUG_EXT_PATH='$(PROJ_DIR)build/debug/extension/duckdb_pgwire/duckdb_pgwire.duckdb_extension'
RELEASE_EXT_PATH='$(PROJ_DIR)build/release/extension/duckdb_pgwire/duckdb_pgwire.duckdb_extension'
